...
for i, google in enumerate(googleplay_paths):
    ...
    with open( google, 'r', encoding = 'utf-16') as file:
        ...
        for row in reader:
            lines.append(' '.join(row[10:12]) + ' ' + row[9])
    
    tot = [line.replace('\n',' ').strip(bad_chars) + '\n' for line in lines[1:] if len(line.strip())>1]
    how_many += len(tot)
    
    with open(join(useful_data_path, f'googleStar {i+1}.txt'), 'w', encoding = 'utf-16') as file:
        file.writelines(tot)
...
with open(appstore_path, 'r', encoding = 'utf-8') as file:
    reader = csv.reader(file, delimiter=',')
    for row in reader:
        lines.append(' '.join(row[7:9]) + ' ' + row[6])

tot = [line.replace('\n',' ').strip(bad_chars) + '\n' for line in lines[1:] if len(line.strip())>1]
how_many += len(tot)
    
with open(join(useful_data_path, f'appstoreStar.txt'), 'w', encoding = 'utf-16') as file:
    file.writelines(tot)
...
if True:
   ...
   for i, google in enumerate(googleplay_paths):
       ...
       with open(join(useful_data_path, f'googleStar {i+1}.txt'), 'r', encoding = 'utf-16') as file:
           lines += file.readlines()
   
   with open(join(useful_data_path, f'appstoreStar.txt'), 'r', encoding = 'utf-16') as file:
       lines += file.readlines()
       
   with open(join(useful_data_path, f'allStar.txt'), 'w', encoding = 'utf-16') as file:
